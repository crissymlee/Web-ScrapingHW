...
def scrape_news():
    ...
    # look for snippet
    article_title = soup.find('div', class_='list_text').find(
        'div', class_='content_title').find('a').text
    article_paragraph = soup.find('div', class_='list_text').find(
        'div', class_='article_teaser_body').text

    # print it
    return article_title, article_paragraph
...
def scrape_image():
    ...
    image_url = soup.find('div', class_='carousel_container').find(
        'article')['style'].replace(');', '')[1:-1]
...
def scrape_hemi():
    ...
    for x in main_url:
        ...
        hemisphere_img_url = hem_imgage_origin.find('a')['href']
